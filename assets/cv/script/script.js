...
class QuarkdownDocument {
  ...
  beforeReady(content) {
    ...
    this.pageMarginInitializers = content.querySelectorAll(
      ".page-margin-content",
    );
  }
...
function applyRemainingHeightProperties() {
  ...
  fillHeightElements.forEach((element) => {
    ...
    const remainingHeight =
      contentArea.getBoundingClientRect().bottom -
      element.getBoundingClientRect().top;

    // Inject CSS variable.
    element.style.setProperty(
      "--viewport-remaining-height",
      `${remainingHeight}px`,
    );
  });
}
...
class PageChunker {
  ...
  isBlank(chunk) {
    return (
      chunk.childNodes.length === 0 ||
      Array.from(chunk.children).every((child) => this.isHidden(child))
    );
  }
  ...
  apply() {
    ...
    if (queuedElements.length > 0 && this.chunks.length > 0) {
      queuedElements.forEach((element) =>
        this.container.lastChild.appendChild(element),
      );
      queuedElements = [];
    }
  }
